...
<div class="card">
  <div class="card-body">
    <ul class="nav nav-pills nav-justified" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" data-toggle="pill" href="#sendmail">{{ _lang('Send Mail') }}</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" data-toggle="pill" href="#groupmail">{{ _lang('Send Mail By Group') }}</a>
      </li>
    </ul>
    ...
    <div class="tab-content">
      <div id="sendmail" class="container tab-pane active"><br>
        {!! Form::open(['route' =>'admin.emailmarketing.sendmail.store', 'class' => 'form-validate-jquery', 'id' => 'content_form', 'files' => true, 'method' => 'POST']) !!}
          <div class="row">
            <div class="col-md-12">
              <div class="card">
                <div class="card-body">
                  <h5 class="card-title">{{ _lang('Compose Mail') }}</h5>
                  <div class="row">
                    <div class="col-md-12">
                      <div class="form-group">
                        {{ Form::label('emails', _lang('Email') , ['class' => 'col-form-label']) }}
                        {!! Form::textarea('emails',null, ['id' => 'emails', 'class' =>'form-control ','required'=>'','placeholder'=>'Email Separated by comma']) !!}
                      </div>
                    </div>
                    <div class="col-md-12">
                      <div class="form-group">
                        {{ Form::label('template', _lang('Email Template') , ['class' => 'col-form-label']) }}
                        {!! Form::select('template', $templates, 'template', [ 'class'=>'form-control select2', 'placeholder' => 'Pick Template','required'=>''])!!}
                      </div>
                    </div>
                    <div class="col-md-12">
                      <div class="form-group">
      ...
      <div id="groupmail" class="container tab-pane fade"><br>
        {!! Form::open(['route' =>'admin.emailmarketing.sendmail.store', 'class' => 'form-validate-jquery ajax_form', 'id' => '', 'files' => true, 'method' => 'POST']) !!}
...
<script>
    ...
    $.ajax({
      url: "{{ url('admin/emailmarketing/get_emails') }}/"+type,
      beforeSend: function(){
        ...
        jQuery.each( json, function( i, val ) {
          $('select[name=client_id]').append("<option value='"+val['email']+"'>"+val['name']+"</option>");
        });
   ...
   function getbrand(type ) {
    $.ajax({
      ...
      beforeSend: function(){
        ...
        jQuery.each( json, function( i, val ) {
          $('select[name=brand_id]').append("<option value='"+val['email']+"'>"+val['name']+"</option>");
        });
    ...
    if($("#client_id").val() == "all"){    
      ...
      $.ajax({
        ...
        beforeSend: function(){
          ...
          jQuery.each( json, function( i, val ) {
             $('#user_list')
             .append('<div class="col-md-12"><div class="animated-checkbox"><label><input type="checkbox" name="client[]" value="'+val['email']+'" checked="true"><span class="label-text">'+val['name']+"-"+val['email']+'</span></label></div></div>');
          });
    ...
    if($("#brand_id").val() == "all"){    
      ...
      $.ajax({
        ...
        beforeSend: function(){
          ...
          jQuery.each( json, function( i, val ) {
             $('#user_list')
             .append('<div class="col-md-12"><div class="animated-checkbox"><label><input type="checkbox" name="brand[]" value="'+val['email']+'" checked="true"><span class="label-text">'+val['name']+"-"+val['email']+'</span></label></div></div>');
          });
