<div class="card">
    <div class="card-header">
        <h6>{{_lang('Edit Occupation Inforamtion - ')}} <span class="badge badge-primary">{{$model->name}} </span> </h6>
    </div>
    <div class="card-body">
        <form action="{{route('admin.setting.member-setting.occupation.update',$model->id)}}" method="POST" id="content_form">
            @method('PATCH')

            <div class="row">    
                {{-- Occupation Name --}}
                <div class="col-md-12 form-group">
                    <label for="name">{{_lang('Occupation Name')}} <span class="text-danger">*</span> </label>
                    <input type="text" name="name" id="name" class="form-control" required placeholder="Enter Occupation Name" value="{{$model->name}} ">
                </div>
                ...
                <div class="col-md-12 form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" placeholder="Enter Occupation Description" cols="30" rows="2">{{$model->description}}</textarea>
                </div>
            </div>
