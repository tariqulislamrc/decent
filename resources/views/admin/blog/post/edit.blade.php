<div class="card">
    <div class="card-header">
        <h6>{{_lang('Edit Blog Post - ')}} <span class="badge badge-primary">{{$model->name}}</span></h6>
    </div>
    <div class="card-body">
        <form action="{{route('admin.eCommerce.blog-post.update', $model->id)}}" method="post" id="content_form">
            @csrf
            @method('PATCH')
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <select required data-placeholder="Select Catagory" name="category" id="category"
                        ...
                        <option {{$item->id == $model->blog_category_id?'selected':''}} value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <input type="text" value="{{$model->title}}" name="title" id="title" class="form-control" placeholder="Enter Blog Post Title"
                        required>
                </div>
                ...
                <div class="col-md-12 form-group">
                    ...
                    <input type="file" name="photo" id="photo" class="dropify"
                        data-default-file="{{isset($model)?asset('storage/blog/'.$model->image):''}}" />
                    <input type="hidden" name="old_photo" value="{{$model->image}}">
                </div>
                ...
                <div class="col-md-12 form-group">
                    ...
                    <textarea name="details"  class="form-control summernote" id="details"
                        placeholder="Enter Post Details">{!!$model->details!!}</textarea>
                </div>
                ...
                <div class="col-md-6 form-group">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="text" class="form-control take_date" name="date" id="date"
                            value="{{ $model->date }}">
                    </div>
                </div>
                ...
                <div class="col-md-6 form-group">
                    ...
                    <select data-placeholder="Category Status" name="status" id="status" class="form-control select">
                        <option {{$model->status == 'Active'?'selected':''}} value="Active">{{_lang('Active')}}</option>
                        <option {{$model->status == 'InActive'?'selected':''}} value="InActive">{{_lang('InActive')}}
                        </option>
                    </select>
                </div>
                ...
                <div class="form-group col-md-12" align="right">
                    ...
                    <button type="button" class="btn btn-info" id="submiting"
                        style="display: none;">{{_lang('Processing')}}<i class="fa fa-spinner fa-spin"
