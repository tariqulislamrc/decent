@extends('layouts.app', ['title' => _lang('eCommerce to Production'), 'modal' => 'lg'])

{{-- Header Section --}}
@section('page.header')
    <div class="app-title">
        <div>
            <h1 data-placement="bottom" title="eCommerce to Production."><i class="fa fa-universal-access mr-4"></i>
                {{_lang('eCommerce to Production')}}</h1>
        </div>
    </div>
@stop
...
@section('content')
    ...
    <form action="{{route('admin.eCommerce.production-to-ecommerce.store_reverse')}}" method="post" id="content_form"
...
<script>
    _formValidation();
           ...
           .autocomplete({
               source: function(request, response) {
                    $.getJSON(
                       '/admin/eCommerce/production-to-ecommerce/search-product',
                       {
               ...
               select: function(event, ui) {
                   if ( ui.item.qty > 0) {
                       $(this).val(null);
                        item1(ui.item, ui.item.variation_id, ui.item.ecommerce_id, 1);
                   } else {
    ...
    //add row function
function item1(item, variation_id, ecommerce_id, quantity) {
    ...
    if (a.length == 0) {
        ...
        $.ajax({
            type: 'GET',
            url: "/admin/eCommerce/production-to-ecommerce/insert",
            data: {
