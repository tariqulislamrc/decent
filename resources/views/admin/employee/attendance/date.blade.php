...
<tr>
    ...
    <td>
        ...
        <span class="font-80pc">
            {{current_designation($model->id)?current_designation($model->id):""}}
            ({{designation_category($model->id)}})
        </span>
    ...
    <td>
        ...
        <select class="form-control select col-12 select_value" name="type[]" id="type" data-placeholder="Select Attendance Type">
            ...
            @foreach ($types as $item)
                ...
                <option {{checkatndance($model->id,$date1) == $item->id ? 'selected' : ''}} {{$holiday == $item->type ? 'selected' : 'not_working'}} value="{{$item->id}}">{{$item->name}} ({{$item->alias}})</option>
