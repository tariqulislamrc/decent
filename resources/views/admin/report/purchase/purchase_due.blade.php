@extends('layouts.app', ['title' => _lang('Purchase Due Report'), 'modal' => 'lg'])
{{-- Header Section --}}
@section('page.header')
<div class="app-title">
	<div>
		<h1 data-placement="bottom" title="Purchase Due Report."><i class="fa fa-universal-access mr-4"></i>
		{{_lang('Report')}}</h1>
	</div>
</div>
@stop
...
<div class="card">
	<div class="card-header">
		<h6>{{_lang('Purchase Due Report')}}</h6>
	</div>
	<div class="card-body">
		<form action="{{route('admin.report.purchasing.purchase_due_report')}}" method="post" enctype="multipart/form-data" target="_blank">
			@csrf
			<div class="row">
				...
				<div class="col-md-6 mx-auto">
					<button type="submit" class="btn btn-block btn-info">{{ _lang('Get Purchase Due Report') }}</button>
				</div>
			</div>
		</form>
	</div>
